...
if not st.session_state.auth:
    ...
    df = st.data_editor(pd.DataFrame([
        {"Cargo":"P1", "L":115, "W":115, "H":115, "Qty":10, "Allow_Rotate": True}
    ...
    if st.button("GENERATE VALIDATED 3D PLAN"):
        ...
        if not clean_df.empty:
            ...
            for i, r in clean_df.iterrows():
                ...
                total_vol += vol
                shipment_vols.append({"name": r['Cargo'], "vol": vol})
                
                # Dim Check
                if not r['Allow_Rotate'] and (r['L'] > specs['L'] or r['W'] > specs['W'] or r['H'] > specs['H']):
                    errors.append(f"❌ {r['Cargo']} exceeds {c_type} limits!")

            # --- METRICS & % BAR ---
            util = (total_vol / specs['MAX_CBM']) * 100
            m1, m2, m3 = st.columns(3)
            with m1: st.markdown(f'<div class="metric-card">Total Cargo Volume<br><h3>{total_vol:.2f} CBM</h3></div>', unsafe_allow_html=True)
            with m2: st.markdown(f'<div class="metric-card">Container Capacity<br><h3>{specs["MAX_CBM"]} CBM</h3></div>', unsafe_allow_html=True)
            with m3: st.markdown(f'<div class="metric-card">Space Utilization<br><h3>{util:.1f}%</h3></div>', unsafe_allow_html=True)
            
            st.write("Storage Load Factor:")
            st.progress(min(util/100, 1.0))

            if total_vol > specs['MAX_CBM']:
                st.error(f"🚨 OVERLOAD! Container capacity exceeded.")
                sorted_list = sorted(shipment_vols, key=lambda x: x['vol'], reverse=True)
                st.info(f"💡 Advice: Holding shipment '{sorted_list[0]['name']}' will save {sorted_list[0]['vol']:.2f} CBM.")
            elif errors:
                for e in errors: st.error(e)
            else:
                # 4. 3D VISUALIZATION (THE MISSING PART)
                fig = go.Figure()
                    ...
                    for _ in range(int(r['Qty'])):
                        ...
                        if cz + h <= CH:
                            fig.add_trace(go.Mesh3d(
                                x=[cx, cx, cx+u_l, cx+u_l, cx, cx, cx+u_l, cx+u_l],
                                y=[cy, cy+u_w, cy+u_w, cy
